...
/**
 * Title: Event Card
 * Slug: bunbukan-europe/bkb-event-card
 * Categories: text
 */

$event_image = bunbukan_europe_find_image( 'DSC01113', array( '/assets/images/' ) );
?>
...
<div class="wp-block-group" style="border-color:var(--wp--preset--color--stone);border-width:1px;padding-top:var(--wp--preset--spacing--lg);padding-right:var(--wp--preset--spacing--lg);padding-bottom:var(--wp--preset--spacing--lg);padding-left:var(--wp--preset--spacing--lg)">
	...
	<figure class="wp-block-image size-full"><img src="<?php echo esc_url( $event_image ); ?>" alt="Event"/></figure>
	<!-- /wp:image -->
	<?php endif; ?>
	<!-- wp:heading {"level":3} -->
	<h3>Official Seminar</h3>
	<!-- /wp:heading -->
	<!-- wp:paragraph {"textColor":"stone"} -->
	<p class="has-stone-color has-text-color">Date • Location</p>
	<!-- /wp:paragraph -->
	<!-- wp:paragraph -->
	<p>Brief description of the event and registration details.</p>
	<!-- /wp:paragraph -->
	<!-- wp:buttons -->
	<div class="wp-block-buttons">
		<!-- wp:button -->
		<div class="wp-block-button"><a class="wp-block-button__link wp-element-button" href="/events">View details</a></div>
		<!-- /wp:button -->
	</div>
	<!-- /wp:buttons -->
</div>
